"""
doji_logic.py (strategy/doji_logic.py)
=====================================
Doji Candle Detection + Entry / SL / Target Strategy Engine

Separate from strategy/logic.py (hammer) so each pattern's rules stay in its own
file. Shared primitives (Candle, TradeSignal, enums, timeframe defaults)
are imported from strategy.logic -- only doji-specific detection and config
live here.
...
def build_trade_signal(
    ...
    return logic.TradeSignal(
        ...
        entry_rule=logic.coerce_entry_rule(config.entry_rule).value,
    )
