"""
hammer_context_core.py (strategy/hammer_context_core.py)
=======================================================
Shared detection + signal builder for the "Hammer with candles" family.

Product patterns (separate strategies — do not merge rules):
  strategy/hammer_with_candles_logic.py      — market entry at rule price
  strategy/hammer_with_candles_35_logic.py   — pullback limit toward SL

This core has NO product label/type and does not coerce pullback %.
Each product module sets entry_pullback_pct before calling run_strategy.
"""
...
def describe_hammer_context_rules(config: HammerContextConfig) -> str:
    ...
    if n <= 0:
        buy_ctx = "no prior-candle context (lookback=0)"
        sell_ctx = "no prior-candle context (lookback=0)"
    else:
        buy_ctx = (
            f"lookback={n}: each of the {n} prior close(s) must stay "
            f">= signal low"
        )
        sell_ctx = (
            f"lookback={n}: each of the {n} prior close(s) must stay "
...
def build_context_signal(
    ...
    return logic.TradeSignal(
        ...
        entry_rule=logic.entry_rule_for_variant(trade_cfg, variant).value,
    )
