"""
logic.py (strategy/logic.py)
============================
Classic / inverted Hammer candle pattern + entry / SL / target engine.
...
class TradeSignal:
    ...
    # EntryRule value used to price entry_price (decides when SL/TP scanning starts)
...
def entry_rule_for_variant(config, variant=None) -> EntryRule:
    """Entry rule that prices a Classic vs Inverted signal."""
    if _is_inverted_variant(variant):
        return coerce_entry_rule(getattr(config, "inverted_entry_rule", config.entry_rule))
    return coerce_entry_rule(config.entry_rule)
...
def calculate_entry_price(
    ...
    """
    rule = entry_rule_for_variant(config, variant)
    if _is_inverted_variant(variant):
...
def build_trade_signal(
    ...
    return TradeSignal(
        ...
        entry_rule=entry_rule_for_variant(config, variant).value,
    )
